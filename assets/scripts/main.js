...
function saveRecipesToStorage(recipes) {
  ...
  //            header. It is possible in only a single line, but should
  //            be no more than a few lines.
  return JSON.stringify(localStorage.setItem('recipes', recipes));
}
...
function initFormHandler() {
  ...
  formEl.addEventListener('submit', () => {
    ...
    let newRecipes = JSON.stringify([...getRecipesFromStorage(), recipeObject]);
    localStorage.setItem('recipes', newRecipes);
  })
  
  // B10. TODO - Get a reference to the "Clear Local Storage" button
  let clsButton = formEl.querySelector('button[type=button]');
